...
> 작성일: 2026-06-22 · 갱신: 2026-06-26 (rev 14 — 결정론 바닥 런타임 확정: Node.js+TypeScript(§8) — Claude Code가 Node라 사용자 추가 의존성 0 · rev 13 lens 수명 티어 §2.4 · rev 12 graph.json §4.7 · rev 11 메타데이터 §2.8) · 상태: 설계 승인 대기
...
- ~~언어 미정~~ → **런타임 확정: Node.js + TypeScript**. Claude Code가 Node 런타임이라 사용자 추가 의존성 0(빌드는 개발자만, 사용자는 컴파일된 JS 실행). scripts는 순수 파싱이라 의존성 최소(프론트매터 파서 정도). 남은 건 정적 파싱 범위·CI 통합·오탐/미탐 임계값.
