...
> 작성일: 2026-06-22 · 갱신: 2026-06-24 (rev 10 — 실행 모드 독립성 §4.6: 정확성 보장/협응은 오케스트레이터 · 트리거 게이트 triviality-aware + report 트리거=plan 기반 §6-A · rev 9 트리비얼 면제 §6-D · rev 8 합격선 §6-K · rev 7 검증 2단계) · 상태: 설계 승인 대기
...
- **adr · plan · report**: `{NNNN}-{YYYY-MM-DD}-{kebab-제목}.md`
  ...
  - **번호는 작업 중 미리 잡지 않는다 (동시성 충돌 회피 — 허점 1)**: 작업 중엔 `{date}-{슬러그}`가 임시 핸들, 일련번호는 **머지 후 `/sherpa-reindex`가 확정**. 두 브랜치가 같은 다음 번호를 읽어도(MADR가 겪은 실제 충돌) reindex가 머지 시점 폴더 INDEX 기준으로 **결정론적 재할당 + 역참조 링크 갱신**. 사람이 머릿속으로 번호를 세지 않는다.
...
- **정체성·신선도**: 타입별 디렉토리(§2.1)에 날짜 스냅샷 커밋. `/sherpa-sweep`이 **타입별 최신 N개만 남기고** 나머지를 `archive/lens/{view}/`로(묘지 누적 차단). 시간축 5종과 역할이 겹치지 않으므로 §2 표 밖.
...
- **graph 파서** (Stop/PostToolUse hook + `/sherpa-reindex`): 역참조 링크를 정적 파싱해 `docs/.graph.json` 산출 — 끊긴 plan↔report 스레드(**plan status로 active-stale vs abandoned 구분** §2.2), plan `done` 자동 파생(report 역참조), supersede 안 된 ADR, 깨진 링크, **spec 재검증 필요(refs 드리프트·last-verified 오래)·spec retire 후보(refs 전부 삭제)**를 **기계가 판정**(LLM 아님). lens·status가 이 사실을 소비(§2.4) — *재검증은 surface만, archive 아님*(§2.7).
...
- **graph.json 질의 계층**(§4.5 — 통째 로드 금지): 에이전트가 필요한 사실만 뽑는 질의 인터페이스 설계 + 산출물 N 증가 시 상시-로드 토큰 *평탄성* 측정·회귀 검사. (graph 파서를 1순위로 끌어올리는 명분 — 사실 산출이자 컨텍스트 경계.)
- **트리비얼 판정 분류기**(§6-D): 코드용 **언어별 공백·부작용 인지** diff 분석 + 문서용 **prose-only 분류**(frontmatter·링크·코드펜스·anchor 제외) — 면제 경계를 결정론으로 구현(LLM 재량 배제).
- NNNN 머지 후 재할당 vs ULID/날짜+슬러그 **최종 채택**(역참조 자동 갱신 비용 비교).
...
- **lens 커밋 범위** — 메뉴 4종(재생성 가능)은 on-demand vs custom만 커밋 — HTML 스냅샷 비대화 측정(§2.4 묘지 누적 경계와 정합).
