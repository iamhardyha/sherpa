...
export function extractLinks(markdown: string): string[] {
  ...
  for (const match of markdown.matchAll(MD_LINK)) {
    ...
    if (target === '') continue; // 순수 앵커(#foo)
    if (/^[a-z][a-z0-9+.-]*:/i.test(target)) continue; // http: mailto: 등 스킴
    links.push(target);
  }
  return links;
}
...
export function checkLinks(markdown: string, fromDir: string, existing: Set<string>): string[] {
  const broken: string[] = [];
  for (const link of extractLinks(markdown)) {
    const resolved = join(fromDir, link);
    if (!existing.has(resolved)) broken.push(resolved);
  }
  return broken;
}
