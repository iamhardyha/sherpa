...
function matches(refPath: string, files: Set<string>): boolean {
  if (files.has(refPath)) return true;
...
export function checkDrift(specs: Node[], changed: Set<string>, existing: Set<string>): Fact[] {
  const facts: Fact[] = [];

  for (const spec of specs) {
    if (spec.type !== 'spec' || !spec.refs) continue;
